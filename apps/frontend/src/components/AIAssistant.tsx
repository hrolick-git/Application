...
import { SparklesIcon, PaperAirplaneIcon, XMarkIcon, LockClosedIcon } from '@heroicons/react/24/outline';
...
export function AIAssistant() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed bottom-24 right-6 z-50 w-[360px] bg-white rounded-[2rem] shadow-2xl border border-slate-100 flex flex-col overflow-hidden"
          style={{ height: '480px' }}
        >
          {/* Header */}
          <div className="p-5 bg-indigo-600 flex items-center gap-3 shrink-0">
          ...
          {!user ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-white">
              ...
              <h4 className="font-black text-slate-800 mb-2">Login Required</h4>
              <p className="text-sm text-slate-500 leading-relaxed">
                The AI Assistant is available for authorized users only. Please log in to ask questions about your events.
              </p>
              <Link
                to="/login"
                className="mt-6 px-6 py-2.5 bg-indigo-600 text-white rounded-2xl text-sm font-bold hover:bg-indigo-700 transition-all"
              >
                Log In
              </Link>
            </div>
            ...
            <>
              ...
              {/* Input */}
              <form onSubmit={handleAsk} className="p-4 border-t border-slate-100 flex gap-2 shrink-0">
                <input
                  value={question}
